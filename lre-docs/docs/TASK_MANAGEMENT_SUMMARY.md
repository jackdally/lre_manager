...
### Feature-Specific Tasks (`active/feature-name.md`)
...
The original `TODO.md` file has been archived as `docs/archive/TODO-LEGACY.md`. The new structure provides:
 ...
 - **Better Organization** - Tasks are grouped by category and feature
 - **Clearer Status Tracking** - Easy to see what's active vs completed
 - **Scalability** - Can handle hundreds of tasks across many features
 - **Maintainability** - Single source of truth for each feature's tasks
 - **Archiving** - Natural lifecycle management for completed work
...
### Adding New Tasks
 - For feature-specific tasks: Add to appropriate feature file in `active/`
 - For bugs: Add to `active/bugs.md` with appropriate priority
 - For general tasks: Add to `active/general.md` with category

### Updating Task Status
 - Update the checkbox status in the appropriate file
...
### Archiving Completed Features
 - Use the helper script: `./scripts/task-management.sh archive <feature-name>`
 - Update the implementation plan status
 - Update `docs/FEATURES.md` to mark the feature as completed

### Creating New Feature Tasks
 - Use the helper script: `./scripts/task-management.sh create-feature <feature-name>`
 - Customize the generated template with specific tasks
 - Link to the corresponding implementation plan
