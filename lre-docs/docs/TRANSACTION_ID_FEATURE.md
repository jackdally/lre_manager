...
#### ImportConfig Entity (`backend/src/entities/ImportConfig.ts`)
...
#### ImportTransaction Entity (`backend/src/entities/ImportTransaction.ts`)
...
#### ImportService (`backend/src/services/importService.ts`)
...
#### ImportPage Component (`frontend/src/components/ImportPage.tsx`)
- Updated `ImportConfig` interface to include `transactionIdColumn?: string`
...
### ✅ **Fields Updated in Ledger Entry:**
 ...
 - **`actual_amount`** - Set to the transaction amount from the import
 - **`actual_date`** - Set to the transaction date from the import  
 - **`notes`** - Set to `"Invoice: {invoiceNumber}"` if an invoice number exists
 - **`invoice_link_url`** - Set to the generated NetSuite URL (NEW)
 - **`invoice_link_text`** - Set to the invoice number or "View Invoice" (NEW)
...
When adding unmatched transactions to the ledger, these fields are also populated:
 - **`vendor_name`** - From the import transaction
 - **`expense_description`** - From the import transaction
 - **`wbs_category`** - User-selected category
 - **`wbs_subcategory`** - User-selected subcategory
 - **`actual_amount`** - From the import transaction
 - **`actual_date`** - From the import transaction
 - **`notes`** - Contains import information and invoice number
 - **`invoice_link_url`** - Set to the generated NetSuite URL (NEW)
 - **`invoice_link_text`** - Set to the invoice number or "View Invoice" (NEW)
...
## Benefits
 ...
 - **Automatic Link Generation**: No need to manually create NetSuite links
 - **Consistent Format**: All links follow the same NetSuite URL pattern
 - **Audit Trail**: Transaction IDs are preserved for reference
 - **Flexibility**: Users can still provide custom Reference Numbers if needed
 - **Backward Compatibility**: Existing uploads without Transaction IDs continue to work
 - **Enhanced Ledger**: Invoice links are now stored directly in ledger entries for easy access
